...
static XtResource resources[] =
{
  ...
  {XtNgainPrimaryCallback, XtCCallback, XtRCallback, sizeof(XtPointer),
	offset(gainPrimaryCallback), XtRCallback, NULL},
  {XtNlosePrimaryCallback, XtCCallback, XtRCallback, sizeof(XtPointer),
	offset(losePrimaryCallback), XtRCallback, NULL},
  {XtNmodifyVerifyCallback, XtCCallback, XtRCallback, sizeof(XtPointer),
	offset(modifyVerifyCallback), XtRCallback, NULL},
  {XtNvalueChangedCallback, XtCCallback, XtRCallback, sizeof(XtPointer),
	offset(valueChangedCallback), XtRCallback, NULL},
  {XtNmotionVerifyCallback, XtCCallback, XtRCallback, sizeof(XtPointer),
	offset(motionVerifyCallback), XtRCallback, NULL},
};
...
static void TfFocusIn(Widget aw, XEvent *event, String *params, Cardinal *num_params), TfFocusOut(Widget aw, XEvent *event, String *params, Cardinal *num_params), HomeChar(Widget aw, XEvent *event, String *params, Cardinal *num_params), EndChar(Widget aw, XEvent *event, String *params, Cardinal *num_params);
...
{
  {
	/* core_class fields     */
		...
		/* destroy               */ (void (*)(struct _WidgetRec *))Destroy,
		/* resize                */ Resize,
...
static void
textfieldCallbacks(TextFieldWidget w, XtCallbackList callbacks,
	int reason, XEvent *event, String str) {
	TextFieldReturnStruct cbs;
	if (callbacks) {
		cbs.reason = reason;
		cbs.event = event;
		cbs.string = str;
		XtCallCallbackList((Widget)w, callbacks, &cbs);
	}
}
...
/* ARGSUSED */
static void
TfFocusIn(Widget aw, XEvent *event, String *params, Cardinal *num_params) {
	TextFieldWidget w = (TextFieldWidget)aw;

	if (w->text.focusCallback)
		textfieldCallbacks(w, w->text.focusCallback, TF_FOCUS, event, w->text.Text);
}

/* ARGSUSED */
static void
TfFocusOut(Widget aw, XEvent *event, String *params, Cardinal *num_params) {
	TextFieldWidget w = (TextFieldWidget)aw;

	if (w->text.losingFocusCallback) {
		TextFieldVerifyStruct cbs;
		cbs.reason = TF_LOSING_FOCUS;
		cbs.event = event;
		cbs.curInsert = cbs.newInsert = w->text.CursorPos;
		cbs.startPos = cbs.endPos = 0;
		cbs.text = NULL;

		XtCallCallbackList(aw, w->text.losingFocusCallback, &cbs);
	}
}
...
Activate(Widget aw, XEvent *event, String *params, Cardinal *num_params) {
	...
	if (w->text.ActivateCallback)
		textfieldCallbacks(w, w->text.ActivateCallback,
			TF_ACTIVATE, event, w->text.Text);
}


	/* Shorthand utility: verify cursor motion */

static	Bool
motionVerify(TextFieldWidget w, XEvent *event, int newpos) {
	TextFieldVerifyStruct cbs;

	if (w->text.motionVerifyCallback == NULL)
		return True;

	  /* allow callback procs to veto */
	  /* TODO: verify that these values are correct. */

	cbs.reason = TF_MOVING_INSERT_CURSOR;
	cbs.event = event;
	cbs.doit = True;
	cbs.curInsert = w->text.CursorPos;
	cbs.newInsert = newpos;
	cbs.startPos = cbs.endPos = -1;
	cbs.text = NULL;
	XtCallCallbackList((Widget)w, w->text.motionVerifyCallback, &cbs);
	return cbs.doit;
}
...
ForwardChar(Widget aw, XEvent *event, String *params, Cardinal *num_params) {
	...
	if (w->text.CursorPos < w->text.TextLen &&
		motionVerify(w, event, w->text.CursorPos + 1)) {
		ClearHighlight(w);
		w->text.CursorPos++;
		EraseCursor(w);
		if (PositionCursor(w))
...
/* ARGSUSED */
static void
HomeChar(Widget aw, XEvent *event, String *params, Cardinal *num_params) {
	TextFieldWidget w = (TextFieldWidget)aw;
...
/* ARGSUSED */
static void
EndChar(Widget aw, XEvent *event, String *params, Cardinal *num_params) {
	TextFieldWidget w = (TextFieldWidget)aw;
	...
	if (w->text.CursorPos < w->text.TextLen &&
		motionVerify(w, event, w->text.TextLen)) {
		ClearHighlight(w);
...
#define INSERTCHARBUFSIZ 32
	...
	if (len > 0) {
		if (w->text.modifyVerifyCallback) {
			TextFieldVerifyStruct cbs;
			/* allow callback procs to veto */
			/* TODO: verify that these values are correct. */
			cbs.reason = TF_MODIFYING_TEXT_VALUE;
			cbs.event = event;
			cbs.doit = True;
			if (w->text.HighlightStart >= 0 && w->text.PendingDelete) {
				cbs.curInsert = w->text.HighlightStart;
				cbs.newInsert = w->text.HighlightStart + len;
				cbs.startPos = w->text.HighlightStart;
				cbs.endPos = w->text.HighlightEnd;
			} else {
				cbs.curInsert = w->text.CursorPos;
				cbs.newInsert = w->text.CursorPos + len;
				cbs.startPos = -1;
				cbs.endPos = -1;
			}
			cbs.text = &textblock;
			...
			textblock.length = len;
			textblock.format = 0;
			XtCallCallbackList(aw, w->text.modifyVerifyCallback, &cbs);
			if (!cbs.doit)
				return;
		}

		EraseCursor(w);
		if (TextInsert(w, textblock.ptr, textblock.length))
			DrawInsert(w);
		else
			Draw(w);

		if (w->text.valueChangedCallback)
			textfieldCallbacks(w, w->text.valueChangedCallback,
				TF_VALUE_CHANGED, event, w->text.Text);
	}
}
...
DeleteNext(Widget aw, XEvent *event, String *params, Cardinal *num_params) {
	...
	if (w->text.modifyVerifyCallback) {
		...
		if (w->text.HighlightStart >= 0 && w->text.PendingDelete) {
			cbs.curInsert = w->text.CursorPos;
			cbs.newInsert = w->text.HighlightStart;
			cbs.startPos = w->text.HighlightStart;
			cbs.endPos = w->text.HighlightEnd;
		} else {
			cbs.curInsert = w->text.CursorPos;
			cbs.newInsert = w->text.CursorPos;
			cbs.startPos = w->text.CursorPos;
			cbs.endPos = w->text.CursorPos;
		}
		cbs.text = NULL;
		XtCallCallbackList(aw, w->text.modifyVerifyCallback, &cbs);
		if (!cbs.doit)
	...
	if (changed && w->text.valueChangedCallback)
		textfieldCallbacks(w, w->text.valueChangedCallback,
			TF_VALUE_CHANGED, event, w->text.Text);
}
...
DeletePrev(Widget aw, XEvent *event, String *params, Cardinal *num_params) {
	...
	if (w->text.modifyVerifyCallback) {
		...
		if (w->text.HighlightStart >= 0 && w->text.PendingDelete) {
			...
			cbs.curInsert = w->text.CursorPos;
			cbs.newInsert = w->text.CursorPos - 1;
			cbs.startPos = w->text.CursorPos - 1;
			cbs.endPos = w->text.CursorPos - 1;
		}
		cbs.text = NULL;
		XtCallCallbackList(aw, w->text.modifyVerifyCallback, &cbs);
		if (!cbs.doit)
...
LoseSelection(Widget aw, Atom *selection) {
	...
	if (*selection == XA_PRIMARY && w->text.losePrimaryCallback) {
		TextFieldReturnStruct ret;
		ret.reason = TF_LOSE_PRIMARY;
		ret.event = NULL;
		ret.string = w->text.Text;
		XtCallCallbackList(aw, w->text.losePrimaryCallback, &ret);
	}
}
...
ExtendEnd(Widget aw, XEvent *event, String *params, Cardinal *num_params) {
	...
	if (len > 0) {
		...
		if (w->text.gainPrimaryCallback != NULL) {
			TextFieldReturnStruct ret;
			ret.reason = TF_GAIN_PRIMARY;
			ret.event = event;
			ret.string = w->text.Text;
			XtCallCallbackList(aw, w->text.gainPrimaryCallback, &ret);
		}
	}
}
