...
#define offset(field) XtOffsetOf(MwListTreeRec, list.field)
...
MwListTreeClassRec listtreeClassRec =
{
  {
	/* core_class fields     */
	/* superclass            */ (WidgetClass) & widgetClassRec,
	/* class_name            */ "MwListTree",
	/* widget_size           */ sizeof(MwListTreeRec),
...
Cardinal *num;
{
  MwListTreeWidget new;
  XGCValues values;
  XtGCMask mask;

  new = (MwListTreeWidget) tnew;
...
Region region;
{
  MwListTreeWidget lw = (MwListTreeWidget) w;
...
/*      w->core.height=height; */
  ...
  if (Layout(w, FALSE, FALSE, &width, &height))
    XtAppWarning(XtWidgetToApplicationContext(w),
     "MwListTree Widget: Size changed when it shouldn't have when resising.");
}
...
{
  MwListTreeWidget lw = (MwListTreeWidget) widget;
  Dimension w, h;
...
#ifdef DEBUG_TREE
void
ItemCheck(MwListTreeWidget w, MwListTreeItem * item)
{
  MwListTreeItem *p;
  char text[1024];
...
void
ChildrenCheck(MwListTreeWidget w, MwListTreeItem * item)
{
  while (item) {
    ItemCheck(w, item);
    if (item->firstchild)
...
void
TreeCheck(MwListTreeWidget w, char *txt)
{
  MwListTreeItem *item;
...
static void
HighlightItem(MwListTreeWidget w, MwListTreeItem * item, Boolean state, Boolean draw)
{
  if (item) {
...
static void
HighlightChildren(MwListTreeWidget w, MwListTreeItem * item, Boolean state, Boolean draw)
{
  while (item) {
...
static void
HighlightAll(MwListTreeWidget w, Boolean state, Boolean draw)
{
  MwListTreeItem *item;
...
static void
HighlightVisibleChildren(MwListTreeWidget w, MwListTreeItem * item, Boolean state, Boolean draw)
{
  while (item) {
    HighlightItem(w, item, state, draw);
...
static void
HighlightAllVisible(MwListTreeWidget w, Boolean state, Boolean draw)
{
  MwListTreeItem *item;
...
static void
AddItemToReturnList(MwListTreeWidget w, MwListTreeItem * item,int loc)
{
  if (loc>=w->list.ret_item_alloc) {
    w->list.ret_item_alloc+=MwListTreeRET_ALLOC;
    w->list.ret_item_list=(MwListTreeItem **)XtRealloc((char *)w->list.ret_item_list,w->list.ret_item_alloc*sizeof(MwListTreeItem *));
  }
  w->list.ret_item_list[loc]=item;
}

static void
MultiAddToReturn(MwListTreeWidget w, MwListTreeItem * item, MwListTreeMultiReturnStruct * ret)
{
...
static void
HighlightCount(MwListTreeWidget w, MwListTreeItem * item, MwListTreeMultiReturnStruct * ret)
{
  while (item) {
    if (item->highlighted)
...
static void
MakeMultiCallbackStruct(MwListTreeWidget w,MwListTreeMultiReturnStruct * ret)
{
  MwListTreeItem *item;
...
static void
HighlightDoCallback(MwListTreeWidget w)
{
...
MwListTreeItem *item;
{
  ...
  size = sizeof(MwListTreeReturnStruct) + ((count + 1) * sizeof(MwListTreeItem *));
  ptr = (char *) XtMalloc(size);
  ret = (MwListTreeReturnStruct *) ptr;
  ret->path = (MwListTreeItem **) (ptr +
				 (((sizeof(MwListTreeReturnStruct) +
				    sizeof(MwListTreeItem *) - 1) /
				   sizeof(MwListTreeItem *)) *
				  sizeof(MwListTreeItem *)));
...
static void
HistoricalCallbacks(MwListTreeWidget w)
{
  MwListTreeReturnStruct *ret;
...
static void
MakeActivateCallbackStruct(MwListTreeWidget w, MwListTreeItem *item,MwListTreeActivateStruct *ret)
{
  int count;
  MwListTreeItem *parent;
...
static void
SelectDouble(MwListTreeWidget w)
{
...
{
  MwListTreeWidget w = (MwListTreeWidget) client_data;
...
Cardinal *num_params;
{
  MwListTreeWidget w = (MwListTreeWidget) aw;
...
/* MwListTree private drawing functions */
...
#ifdef USE_RDD
/* --------------------------------------------------------------------------
   ** MwListTree drag & drop operations
 */
...
Cardinal *num_params;
{
  ...
  if (item) {
    ...
    rddSetDropDataType(ret, sizeof(MwListTreeReturnStruct),
		       RDD_LISTTREE_TYPE);
...
/* ARGSUSED */
void
MwListTreeHighlightDrop(w, event, params, num_params)
Widget w;
XEvent *event;
String *params;
Cardinal *num_params;
...
MwListTreeGetDrop(Widget w)
{
  ...
  if (lw->list.drop_highlighted) {
    ...
    MwListTreeHighlightDrop(w, NULL, NULL, 0);
  }
  else
    ret = NULL;
  return ret;
...
void
MwListTreeRefresh(MwListTreeWidget w)
{
...
void
MwListTreeRefreshOff(MwListTreeWidget w)
{
  w->list.Refresh = False;
}

void
MwListTreeRefreshOn(MwListTreeWidget w)
{
  w->list.Refresh = True;
  MwListTreeRefresh(w);
}
...
MwListTreeItem *
MwListTreeAdd(MwListTreeWidget w,MwListTreeItem *parent,char *string)
{
  MwListTreeItem *item;
  int len;
  char *copy;
  ...
  len = strlen(string);
  item = (MwListTreeItem *) XtMalloc(sizeof(MwListTreeItem));
...
void
MwListTreeRenameItem(MwListTreeWidget w,MwListTreeItem *item,char *string)
{
  int len;
  char *copy;
...
int
MwListTreeDelete(MwListTreeWidget w,MwListTreeItem *item)
{
  if (item->firstchild)
    DeleteChildren(w, item->firstchild);
  item->firstchild = NULL;
...
int
MwListTreeDeleteChildren(MwListTreeWidget w,MwListTreeItem *item)
{
  if (item->firstchild)
    DeleteChildren(w, item->firstchild);
  item->firstchild = NULL;
...
int
MwListTreeReparent(MwListTreeWidget w, MwListTreeItem *item,MwListTreeItem *newparent)
{
...
int
MwListTreeReparentChildren(MwListTreeWidget w, MwListTreeItem *item,MwListTreeItem *newparent)
{
...
MwListTreeUserOrderSiblings(MwListTreeWidget w, MwListTreeItem *item,int (*func)())
{
  ...
  TreeCheck(w, "in MwListTreeUserOrderSiblings");
...
/* Count the children */
  ...
  size = sizeof(MwListTreeItem *);
  list = (MwListTreeItem **) XtMalloc(size * count);
  list[0] = first;
  count = 1;
  ...
  MwListTreeRefresh(w);
  TreeCheck(w, "exiting MwListTreeOrderSiblings");

  return 1;
}

int
MwListTreeOrderSiblings(MwListTreeWidget w, MwListTreeItem *item)
{
  TreeCheck(w, "in MwListTreeOrderSiblings");
  return MwListTreeUserOrderSiblings(w, item, AlphabetizeItems);
}
...
MwListTreeUserOrderChildren(MwListTreeWidget w, MwListTreeItem *item,int (*func)())
{
  ...
  if (item) {
    first = item->firstchild;
    if (first)
      MwListTreeUserOrderSiblings(w, first, func);
  }
  else {
    if (w->list.first)
      MwListTreeUserOrderSiblings(w, w->list.first, func);
  }
...
int
MwListTreeOrderChildren(MwListTreeWidget w, MwListTreeItem *item)
{
  MwListTreeItem *first;

  TreeCheck(w, "in MwListTreeOrderChildren");
  if (item) {
    first = item->firstchild;
    if (first)
      MwListTreeOrderSiblings(w, first);
  }
  else {
    if (w->list.first)
      MwListTreeOrderSiblings(w, w->list.first);
  }
...
MwListTreeItem *
MwListTreeFindSiblingName(MwListTreeWidget w, MwListTreeItem *item, char *name)
{
  MwListTreeItem *first;
...
MwListTreeItem *
MwListTreeFindChildName(MwListTreeWidget w, MwListTreeItem *item, char *name)
{
...
void
MwListTreeHighlightItem(MwListTreeWidget w, MwListTreeItem *item)
{
...
void
MwListTreeHighlightAll(MwListTreeWidget w)
{
...
void
MwListTreeClearHighlighted(MwListTreeWidget w)
{
  HighlightAll(w,False,False);
  MwListTreeRefresh(w);
}
...
void
MwListTreeGetHighlighted(Widget w,MwListTreeMultiReturnStruct *ret)
{
  if (ret) MakeMultiCallbackStruct((MwListTreeWidget)w,ret);
}

void
MwListTreeSetHighlighted(Widget mw,MwListTreeItem **items,int count,Boolean clear)
{
  MwListTreeWidget w = (MwListTreeWidget)mw;
...
Position
MwListTreeGetItemPosition(MwListTreeWidget w, MwListTreeItem *item)
{
  return GetPosition(w, item);
}

void
MwListTreeGetPathname(MwListTreeReturnStruct * ret, char *dir)
{
  int count;
...
void
MwListTreeGetPathnameFromItem(MwListTreeItem *item, char *dir)
{
  char tmppath[1024];
